...
export class LuaComicProvider implements SourceProvider {
  ...
  async fetchPopular(
    ...
    try {
      const url = `https://luacomic.org/manga/page/${page}/?m_orderby=views`;
      const html = await invoke<string>("fetch_html", { url, headers: null });
      return this.parseMadaraList(html, limit, "luacomic.org");
    } catch (e) {
      console.warn("[LuaComic] fetchPopular failed:", e);
      return [];
    }
  }
  ...
  async fetchLatest(
    ...
    try {
      const url = `https://luacomic.org/manga/page/${page}/?m_orderby=latest`;
      const html = await invoke<string>("fetch_html", { url, headers: null });
      return this.parseMadaraList(html, limit, "luacomic.org");
    } catch (e) {
      console.warn("[LuaComic] fetchLatest failed:", e);
      return [];
    }
  }
}
