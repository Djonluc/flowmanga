...
export class KonachanClient extends BaseProvider {
  readonly id = "konachan";
  readonly name = "Konachan";
  readonly baseUrl = "https://konachan.com";
  readonly domains = ["konachan.com"];
  ...
  async search(query: StructuredQuery, options: EngineSearchOptions): Promise<ImageMedia[]> {
    ...
    const data = await this.fetchJson<KonachanPost[]>("/post.json", {
  ...
  private mapPosts(posts: KonachanPost[]): ImageMedia[] {
