...
export const TopBar = () => {
    ...
    return (
        <div className="relative pt-3 pb-2">
            <div className="min-h-16 flex items-center justify-between gap-3 lg:gap-6 px-4 sm:px-6 lg:px-8 xl:px-10 sticky top-0 z-30 no-drag bg-transparent">
            ...
            <div className="min-w-0 flex-1 max-w-3xl">
                <div className="relative group/search">
                    ...
                    />
                    <div className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface border border-border-subtle text-[10px] text-foreground-muted font-black uppercase tracking-widest shadow-lg">
                        <Command size={12} />
                        <span>K</span>
                    </div>
                </div>
            ...
            <div className="flex shrink-0 items-center gap-1 sm:gap-2 lg:gap-3">
                ...
                <div className="hidden sm:block w-px h-8 bg-border-subtle mx-1" />
                
                {/* Download Indicator */}
                <button 
                    onClick={toggleDownloadPanel}
                    className={clsx(
                        "relative p-2.5 sm:p-3 rounded-2xl transition-all border group/dl shadow-lg overflow-visible",
                ...
                >
                    {activeJobs.length > 0 && (
                        <div 
                            className="absolute bottom-0 left-0 w-full rounded-b-2xl bg-blue-500/30 transition-all duration-300 ease-linear z-0 pointer-events-none"
                    ...
                    {hasDownloads && (
                        <span className="absolute -top-1.5 -right-1.5 min-w-5 h-5 px-1 bg-accent text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-lg shadow-accent-glow border-2 border-background z-20">
...
const TopBarButton = ({ icon, title, onClick, active }: any) => (
    <button 
        onClick={onClick}
        className={clsx(
            "p-2.5 sm:p-3 rounded-2xl transition-all border border-transparent shadow-lg",
