...
import { FolderOpen, PlusCircle, LayoutGrid, Library as LibraryIcon, Tag, Edit2, Trash2, X, Sparkles, Bell, Heart, Layers, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const LibraryGrid = () => {
    ...
    const { activeView, libraryViewMode, libraryDensity, setLibraryViewMode, setLibraryDensity, isInitializing } = useSettingsStore();
    ...
    const handleAction = async (action: 'tag' | 'rename' | 'delete' | 'refresh' | 'favorite', item: any) => {
        setActiveMenu(null);
        if (action === 'tag') {
            ...
            toast.success(item.tags?.includes('favorite') ? 'Removed from favorites' : 'Added to favorites');
        }
    };
    ...
    const getHeaderInfo = () => {
        switch (activeView) {
            case 'updates': return { title: 'Recent Updates', icon: <Bell className="text-accent" /> };
            case 'favorites': return { title: 'My Favorites', icon: <Heart className="text-accent" /> };
            case 'collections': return { title: 'Collections', icon: <Layers className="text-accent" /> };
            default: return { title: 'My Library', icon: <LibraryIcon className="text-accent" /> };
        }
    };
        ...
        <div className="h-full relative overflow-hidden">
            <AnimatePresence mode="wait">
                {selectedSeriesId ? (
                    ...
                    >
                        {series.length === 0 ? (
                            <EmptyState onImport={() => openImportModal()} onLink={handleSelectLibrary} />
                        ) : (
                            <div 
                            ...
                            >
                                ...
                                <div className="flex flex-col gap-6 mb-4 px-4 pt-2 z-10 shrink-0">
                                    ...
                                    <div className="flex flex-col gap-4 px-1 pb-2">
                                        ...
                                        <div className="flex items-center gap-4 bg-white/[0.02] border border-white/5 p-3 rounded-2xl relative group/tags">
                                            <div className="flex items-center gap-2 flex-shrink-0 pr-4 border-r border-white/5">
                                                <Tag size={14} className="text-indigo-400" />
                                                <span className="text-[10px] font-black uppercase tracking-widest text-neutral-500 whitespace-nowrap">Explore Tags</span>
                                            </div>
                                                ...
                                                >
                                                    {allTags.filter(t => !filterTags.includes(t)).map(tag => (
                                                        <button
                                                            key={tag}
                                                            onClick={() => toggleFilterTag(tag)}
                                                            className="flex-shrink-0 px-4 py-2 rounded-xl text-[11px] font-bold tracking-tight bg-white/[0.03] border border-white/5 text-neutral-400 hover:text-white hover:bg-white/10 hover:border-white/10 transition-all active:scale-95"
                                                        >
                                                            {tag}
                                                        </button>
                                                    ))}
                                                    {allTags.length === 0 && (
                                                        <span className="text-[10px] text-neutral-600 font-medium">No tags available yet</span>
                                                    )}
                                                </div>

                                                {/* Scroll Buttons */}
                                                <div className="absolute right-0 top-0 bottom-0 flex items-center gap-1 pl-8 bg-gradient-to-l from-[#0a0a0a] via-[#0a0a0a]/80 to-transparent opacity-0 group-hover/tags:opacity-100 transition-opacity">
                                                    <button onClick={() => scrollTags('left')} className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-neutral-400 hover:text-white transition-all"><ChevronLeft size={14} /></button>
                                                    <button onClick={() => scrollTags('right')} className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-neutral-400 hover:text-white transition-all"><ChevronRight size={14} /></button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-12 mb-20 px-4">
                                        <div className="flex items-center justify-between mb-8">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">Global Discoveries</h3>
                                            </div>
                                            {isSearchingExternal && (
                                        ...
                                        {externalResults.length > 0 ? (
                                                    ...
                                                    >
                                                        <div className="aspect-[2/3] rounded-2xl overflow-hidden border border-white/5 bg-white/5 relative mb-3">
                                                            ...
                                                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                                                                <span className="text-[10px] font-black text-white uppercase tracking-widest">View Info</span>
                                                            </div>
                                            ...
                                            <div className="p-12 border border-dashed border-white/5 rounded-[32px] flex flex-col items-center justify-center text-center opacity-40">
                                                ...
                                                <p className="text-xs font-bold text-neutral-600 uppercase tracking-widest">No web matches for these tags</p>
                                            </div>
                                        )}
                                    </div>
                                )}
...
const EmptyState = ({ onImport, onLink }: { onImport: () => void, onLink: () => void }) => (
    <div className="flex flex-col items-center justify-center h-full p-12 bg-black/40 backdrop-blur-md">
        ...
        >
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 bg-indigo-500/10 blur-[100px] rounded-full -z-10 animate-pulse" />
            <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-indigo-600 to-purple-600 p-6 shadow-2xl shadow-indigo-500/20 mb-8 transform -rotate-6">
                <FolderOpen size={48} className="text-white" />
            </div>
            <h1 className="text-4xl font-bold text-white tracking-tight mb-4">Library <span className="text-indigo-500">Empty</span></h1>
            <p className="text-neutral-500 font-medium text-sm tracking-wide leading-relaxed mb-10 max-w-sm">Link your collection to begin reading.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                <button onClick={onImport} className="p-6 rounded-[24px] bg-white/5 hover:bg-white/10 border border-white/5 transition-all text-left group">
                    <PlusCircle size={24} className="text-indigo-500 mb-3 group-hover:scale-110 transition-transform" />
                    <span className="block text-white font-semibold text-xs tracking-wide">Add Series</span>
                </button>
                <button onClick={onLink} className="p-6 rounded-[24px] bg-indigo-600 hover:bg-indigo-500 shadow-xl shadow-indigo-600/20 transition-all text-left group">
                    <FolderOpen size={24} className="text-white mb-3 group-hover:scale-110 transition-transform" />
                    <span className="block text-white font-semibold text-xs tracking-wide">Link Root</span>
                </button>
            </div>
        </motion.div>
    </div>
...
const ContextMenu = ({ activeMenu, onAction, onClose }: { activeMenu: any, onAction: (a: any, i: any) => void, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100]" onClick={onClose}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} style={{ left: activeMenu.x, top: activeMenu.y }} className="absolute bg-neutral-900 border border-white/10 rounded-xl shadow-2xl py-2 min-w-[200px] backdrop-blur-xl">
                <button onClick={() => onAction('tag', activeMenu.item)} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 text-white hover:bg-white/10 transition-colors"><Tag size={16} className="text-indigo-400" />Manage Tags</button>
                <button onClick={() => onAction('rename', activeMenu.item)} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 text-white hover:bg-white/10 transition-colors"><Edit2 size={16} className="text-purple-400" />Rename Series</button>
                <button onClick={() => onAction('refresh', activeMenu.item)} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 text-white hover:bg-white/10 transition-colors"><Sparkles size={16} className="text-accent" />Refresh Metadata</button>
                <button onClick={() => onAction('favorite', activeMenu.item)} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 text-white hover:bg-white/10 transition-colors"><Heart size={16} className="text-red-400" />Add to Favorites</button>
                <div className="h-px bg-white/5 my-1" />
                <button onClick={() => onAction('delete', activeMenu.item)} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={16} />Delete Series</button>
            </motion.div>
        </div>
    );
}
