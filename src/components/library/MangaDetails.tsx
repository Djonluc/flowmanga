...
import { 
    ...
    Globe, Edit2, Bomb, Calendar, BookOpen, Download, Plus, Layers,
    History, CheckCircle2, AlertCircle, ChevronRight, Share2, Bookmark,
    ArrowUpRight, HardDrive, ListFilter, LayoutGrid, Info, MoreHorizontal,
    Heart, Share, Database, PieChart, ArrowDownToLine, MoreVertical,
    FolderOpen, Trash, RotateCcw, FileOutput, Search, X
} from 'lucide-react';
...
export const MangaDetails: React.FC<MangaDetailsProps> = ({ seriesId, onBack }) => {
  ...
  const latestWithProgress = useMemo(() => {
    return [...sortedChapters].sort((a, b) => {
        const numA = parseFloat(a.meta.chapter || '0');
        const numB = parseFloat(b.meta.chapter || '0');
        return numA - numB;
    }).filter(b => b.progress && b.progress.currentPage > 0).pop();
  }, [sortedChapters]);

  const readCount = sortedChapters.filter(b => b.progress && b.progress.currentPage >= (b.progress.totalPages - 2)).length;
  ...
  const missingFromRemote = remoteChapters.filter(rc => !sortedChapters.some(c => c.meta.chapter === rc.number.toString()));

  const filteredChapters = useMemo(() => {
    let base = [...sortedChapters, ...missingFromRemote.map(rc => ({ ...rc, isMissing: true }))].sort((a, b) => {
        const numA = parseFloat((a as any).meta?.chapter || (a as any).number || '0');
        const numB = parseFloat((b as any).meta?.chapter || (b as any).number || '0');
        return numB - numA;
    });
  ...
  return (
    ...
    >
      ...
      <div className="z-50 p-6 flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 text-foreground-muted hover:text-foreground transition-colors text-xs font-bold uppercase tracking-widest group bg-surface-elevated px-4 py-2 rounded-xl backdrop-blur-xl border border-border-subtle">
              ...
              <div className="relative">
                <button 
                    ...
                    className={clsx(
                        ...
                        isTopMoreMenuOpen ? "bg-foreground text-background border-foreground" : "bg-surface-elevated border-border-subtle text-foreground-muted hover:text-foreground"
                    )}
                >
                ...
                <AnimatePresence>
                    {isTopMoreMenuOpen && (
                        <>
                            <div className="fixed inset-0 z-40" onClick={() => setIsTopMoreMenuOpen(false)} />
                            <motion.div 
                            ...
                            >
                                <MenuAction icon={<FolderOpen size={14} />} label="Open Manga Folder" onClick={() => {
        ...
        <div className="max-w-[1600px] mx-auto space-y-12">
             ...
             <div className="relative group shrink-0 self-start">
                 <div className="w-[280px] lg:w-[320px] aspect-[2/3] rounded-3xl overflow-hidden shadow-cinematic border border-border-subtle relative transition-transform duration-700 group-hover:scale-[1.02]">
                     {coverSrc ? (
                         ...
                         <div className="w-full h-full bg-surface-elevated flex items-center justify-center"><LibraryIcon size={64} className="text-foreground-dim" /></div>
                     )}
             ...
             <div className="flex-1 min-w-0 space-y-10 py-2">
                <div className="space-y-6 relative">
                    ...
                    <div className="flex flex-wrap items-center gap-x-12 gap-y-6 relative">
                        <MetadataItem label="Architect" value={selectedSeries.author || 'Unknown'} highlight />
                        <MetadataItem label="Visualizer" value={selectedSeries.artist || 'Anonymous'} />
                        <MetadataItem label="Chronology" value={`${totalCount} Chapters`} />
                        <MetadataItem 
                ...
                <div className="relative group/synopsis max-w-4xl">
                    <p className={clsx(
                        ...
                        {selectedSeries.description || "The archives are silent on this series' history. Awaiting manifestation..."}
                    </p>
                    <button 
                    ...
                    >
                        ...
                        <ChevronRight size={14} className={clsx("transition-transform", synopsisExpanded ? "-rotate-90" : "group-hover:translate-x-1")} />
                    </button>
                </div>
             </div>
             ...
             <div className="w-full lg:w-[320px] shrink-0">
                <div className="bg-surface-elevated border border-border-subtle rounded-[40px] p-10 space-y-10 backdrop-blur-3xl shadow-cinematic relative overflow-hidden group/vital">
                    ...
                    <div className="space-y-8 relative">
                        <InfoItem icon={<Globe size={18} />} label="Archive Status" value={selectedSeries.status || 'Active'} valueClass="text-accent" />
                        <InfoItem icon={<Layers size={18} />} label="Total Scrolls" value={totalCount.toString()} />
                        <InfoItem icon={<HardDrive size={18} />} label="Archive Weight" value="1.24 GB" />
                        <InfoItem icon={<Calendar size={18} />} label="First Signal" value="2025" />
                        <InfoItem icon={<Clock size={18} />} label="Last Resonated" value={new Date(selectedSeries.updatedAt).toLocaleDateString()} />
                        <InfoItem icon={<History size={18} />} label="Read Duration" value="14h 22m" />
                    </div>
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-3 flex gap-4 h-28">
                  ...
                  >
                      <Heart size={24} fill={selectedSeries.tags.includes('favorite') ? "currentColor" : "none"} className="group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] font-black uppercase tracking-[0.2em]">{selectedSeries.tags.includes('favorite') ? 'Favorited' : 'Favorite'}</span>
                  </button>
                  <div className="relative">
                      <button 
                        onClick={() => setIsActionsMoreMenuOpen(!isActionsMoreMenuOpen)}
                        className={clsx(
                            ...
                            isActionsMoreMenuOpen ? "bg-foreground text-background" : "text-foreground-dim hover:text-foreground"
                        )}
                      >
                      ...
                      <AnimatePresence>
                        {isActionsMoreMenuOpen && (
                            <>
                                <div className="fixed inset-0 z-40" onClick={() => setIsActionsMoreMenuOpen(false)} />
                                <motion.div 
                                    initial={{ opacity: 0, scale: 0.9, y: 10 }}
                                    animate={{ opacity: 1, scale: 1, y: 0 }}
                                    exit={{ opacity: 0, scale: 0.9, y: 10 }}
          ...
          <div className="bg-surface-elevated border border-border-subtle rounded-[32px] p-10 flex items-center justify-between gap-12 backdrop-blur-xl relative overflow-hidden group/prog shadow-cinematic">
             ...
             <div className="flex items-center gap-6 shrink-0 relative">
                ...
                <span className="text-[11px] font-black uppercase tracking-[0.5em] text-foreground-muted">Archival Resonance</span>
             </div>
             ...
             <div className="flex items-center gap-16 shrink-0 relative">
                 <span className="text-4xl font-black italic tracking-tighter text-foreground">{progressPercent}%</span>
                 <div className="w-px h-10 bg-border-subtle" />
                 <div className="text-right">
                    <div className="text-2xl font-black italic uppercase tracking-tighter text-accent">Chapter {currentChapterDisplay}</div>
                    <div className="text-[10px] text-foreground-dim font-bold uppercase tracking-widest mt-1">of {totalCount} chapters manifested</div>
                 </div>
             </div>
          </div>
             ...
             <div className="flex-1 space-y-10 min-w-0">
                <div className="flex items-center justify-between border-b border-white/5 pb-8">
                    <div className="flex items-center gap-12">
                        <h2 className="text-3xl font-black italic uppercase tracking-tighter flex items-center gap-4">
                            Archived Scrolls <span className="px-3 py-1 rounded-xl bg-white/10 text-sm font-bold not-italic text-accent">{totalCount}</span>
                        </h2>
                        <div className="flex items-center bg-white/5 rounded-2xl p-1.5 border border-white/5">
                            <TabButton active={activeTab === 'all'} label="All Scrolls" onClick={() => setActiveTab('all')} />
                            <TabButton active={activeTab === 'downloaded'} label="Manifested" onClick={() => setActiveTab('downloaded')} />
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="flex items-center bg-surface-elevated rounded-xl p-1 border border-border-subtle mr-4">
                            ...
                            <button 
                                ...
                                className={clsx("p-2.5 transition-colors rounded-lg", isSearching ? "text-accent bg-accent/10" : "text-foreground-muted hover:text-foreground")}
                            >
                ...
                <div className={clsx(
                    ...
                    {filteredChapters.map((item: any, idx) => {
                        if (item.isMissing) {
                            return <MissingChapterCard key={`missing-${item.id || item.number}`} item={item} seriesId={selectedSeries.id} />;
                        }

                        const isRead = item.progress && item.progress.currentPage >= item.progress.totalPages - 2;
                        const isCurrent = latestWithProgress?.id === item.id;
                        const percent = item.progress ? Math.floor((item.progress.currentPage / item.progress.totalPages) * 100) : 0;
                        const chCover = item.cover ? (item.cover.startsWith('http') ? item.cover : convertFileSrc(item.cover)) : null;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="ml-8 flex-1 min-w-0">
                                    <div className="flex items-center gap-4">
                                        <span className="text-foreground-dim font-black italic text-sm tracking-tighter w-8">{item.meta.chapter || idx + 1}</span>
                                        <h4 className="text-foreground font-black text-base truncate uppercase italic tracking-tight group-hover:text-accent transition-colors">Scroll {item.meta.chapter || idx + 1}</h4>
                                        {idx === 0 && (
                                            <span className="px-2 py-0.5 rounded-lg bg-accent/20 text-accent text-[9px] font-black uppercase tracking-[0.2em] animate-pulse">NEW</span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-6 mt-2 text-[10px] font-black uppercase tracking-[0.2em] text-foreground-dim group-hover:text-foreground-muted transition-colors">
                                        <span className="flex items-center gap-2"><Clock size={12} /> {new Date(item.updatedAt || selectedSeries.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                        {isCurrent ? <span className="text-accent">{percent}% Synchronized</span> : <span>{item.totalPages || '?'} Pages</span>}
                                    </div>
                                </div>
             ...
             <div className="w-full xl:w-[420px] space-y-12 shrink-0">
                ...
                <SidebarPanel title="Archive Vitality">
                    <div className="space-y-6">
                        ...
                        <div className="pt-2">
                             <StorageLabel dot="bg-accent" label="Image Manifests" value="1.18 GB" />
                             <div className="h-4" />
                             <StorageLabel dot="bg-neutral-600" label="Auxiliary Data" value="60 MB" />
                        </div>
                    </div>
                </SidebarPanel>
                        ...
                        <div className="relative w-32 h-32 shrink-0 group/storage">
                            <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                                ...
                                <motion.circle 
                                    ...
                                    cx="18" cy="18" r="16" fill="none" className="stroke-accent" strokeWidth="3" strokeLinecap="round" 
                                />
                            </svg>
                  ...
                  >
                      ...
                      <div className="space-y-10">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <DownloadOption icon={<Sparkles size={18} />} label="Manifest Missing" sub="Only fill archive gaps" onClick={() => {
                                  toast.success("Summoning missing scrolls...");
                                  setIsDownloadSelectorOpen(false);
                                  UpdateManager.checkForUpdates(selectedSeries.id);
                              }} />
                              <DownloadOption icon={<ArrowDownToLine size={18} />} label="Manifest Latest" sub="Get the newest scrolls" onClick={() => {
                              ...
                              <div className="flex items-center gap-6">
                                  <div className="flex-1 space-y-2">
                                      ...
                                      <input 
                                        ...
                                        onChange={(e) => setDownloadRange({ ...downloadRange, start: e.target.value })}
                                      />
                                  </div>
                                  ...
                                  <div className="flex-1 space-y-2">
                                      ...
                                      <input 
                                        ...
                                        onChange={(e) => setDownloadRange({ ...downloadRange, end: e.target.value })}
                                      />
                                  </div>
                              </div>
                              <button 
                                onClick={async () => {
                                    ...
                                    toast.success(`Manifesting ${toDownload.length} Scrolls...`);
                                    setIsDownloadSelectorOpen(false);
                                    ...
                                    await UpdateManager.downloadChapters(selectedSeries.id, chapters);
                                } }
...
const MenuAction = ({ icon, label, onClick, danger }: any) => (
    <button 
        ...
        className={clsx(
            ...
            danger ? "hover:bg-red-500/10 text-red-500/80 hover:text-red-500" : "hover:bg-accent/10 text-foreground-dim hover:text-accent"
        )}
    >
        <div className={clsx("transition-transform group-hover/item:rotate-12", danger ? "text-red-500" : "text-accent")}>{icon}</div>
...
const MetadataItem = ({ label, value, highlight }: any) => (
    <div className="space-y-1 group/meta">
        <span className="text-[10px] text-foreground-dim font-black uppercase tracking-[0.4em] group-hover:text-accent transition-colors">{label}</span>
        <div className={clsx("text-sm font-bold tracking-wide drop-shadow-md", highlight ? "text-accent" : "text-foreground-muted group-hover:text-foreground transition-colors")}>
            {value}
        </div>
    </div>
);
...
const InfoItem = ({ icon, label, value, valueClass }: any) => (
    <div className="flex items-center gap-5 group/info">
        <div className="w-12 h-12 rounded-2xl bg-surface-raised flex items-center justify-center text-foreground-dim group-hover/info:bg-accent-soft group-hover/info:text-accent transition-all duration-500 shadow-sm">{icon}</div>
        <div>
            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-foreground-dim">{label}</div>
            <div className={clsx("text-sm font-black uppercase tracking-tight mt-1", valueClass || "text-foreground")}>{value}</div>
        </div>
    </div>
);
...
const ActionCard = ({ icon, label, sub, onClick, loading }: any) => (
    ...
    >
        ...
        <div className="text-left relative">
            <div className="text-base font-black uppercase tracking-widest text-foreground group-hover:text-accent transition-colors">{label}</div>
            <div className="text-[10px] text-foreground-dim font-bold uppercase tracking-widest mt-1 italic">{sub}</div>
        </div>
    </button>
);
...
const TabButton = ({ active, label, onClick }: any) => (
    <button 
        ...
        className={clsx(
            ...
            active ? "bg-foreground text-background shadow-cinematic" : "text-foreground-dim hover:text-foreground"
        )}
    >
        {label}
    </button>
...
const SidebarItem = ({ label, value }: any) => (
    <div className="space-y-2">
        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground-dim block">{label}</span>
        <span className="text-sm font-bold text-foreground-muted block leading-relaxed tracking-wide italic">{value}</span>
    </div>
);
...
const StatItem = ({ icon, label, value, color }: any) => (
    <div className="flex items-center justify-between group cursor-default">
        <div className="flex items-center gap-4">
            <div className={clsx("w-10 h-10 rounded-xl bg-surface-raised flex items-center justify-center text-foreground-dim group-hover:bg-accent-soft group-hover:text-accent transition-all duration-500 shadow-sm", color)}>{icon}</div>
            <span className="text-xs font-bold text-foreground-dim uppercase tracking-widest group-hover:text-foreground transition-colors">{label}</span>
        </div>
        <span className="text-sm font-black italic text-foreground-muted group-hover:text-foreground transition-colors">{value}</span>
    </div>
);
...
const StorageLabel = ({ dot, label, value }: any) => (
    <div className="flex items-center justify-between group">
        <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] font-black uppercase tracking-widest text-foreground-dim group-hover:text-foreground transition-colors">{label}</span>
        </div>
        <span className="text-[11px] font-black italic text-foreground-muted group-hover:text-foreground transition-colors">{value}</span>
    </div>
);

const MissingChapterCard = ({ item, seriesId }: { item: any, seriesId: string }) => {
    ...
    return (
        <div className="group relative flex items-center p-5 bg-surface/50 border border-border-subtle border-dashed rounded-[24px] transition-all opacity-60 hover:opacity-100 hover:border-accent shadow-cinematic">
            ...
            <div className="ml-8 flex-1 min-w-0">
                <div className="flex items-center gap-4">
                    <span className="text-foreground-dim font-black italic text-sm tracking-tighter w-8">{item.number}</span>
                    <h4 className="text-foreground-muted font-black text-base truncate uppercase italic tracking-tight group-hover:text-foreground transition-colors">Scroll {item.number}</h4>
                </div>
                <div className="text-[10px] text-foreground-dim font-black uppercase tracking-[0.2em] mt-2 italic">Missing from Archive</div>
            </div>
            <button 
                onClick={async (e) => {
                    e.stopPropagation();
