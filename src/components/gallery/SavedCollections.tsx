...
export const SavedCollections: React.FC = () => {
  const {
    savedImages, folders,
    createFolder, deleteFolder, togglePinFolder,
    likeImage, unlikeImage, openViewer
  } = useGalleryStore();
  const { openInputModal } = useModalStore();

  const likedImages = savedImages.filter(i => i.liked);
  const unsortedImages = savedImages.filter(i => !i.folderId);
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-black text-foreground uppercase tracking-tighter italic">Saved Collections</h3>
      ...
      {likedImages.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-sm font-black text-foreground uppercase tracking-wider">Liked ({likedImages.length})</h4>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {likedImages.slice(0, 12).map((img, idx) => (
              <GalleryImageCard key={img.id} id={img.id} imageUrl={img.imageUrl} previewUrl={img.previewUrl}
                title={img.tags[0]} tags={img.tags} liked={true} saved={true}
      ...
      {folders.length > 0 && (
        <section>
          <h4 className="text-sm font-black text-foreground uppercase tracking-wider mb-4">Folders</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {folders.map(folder => {
              const folderImages = savedImages.filter(i => i.folderId === folder.id);
              return (
                <motion.div key={folder.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2.5">
                      {folder.pinned && <Pin size={12} className="text-amber-500" />}
                      <h5 className="text-base font-bold text-foreground truncate">{folder.name}</h5>
                    </div>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => togglePinFolder(folder.id)} className="p-1.5 text-foreground-dim hover:text-amber-500 transition-colors">
                        <Pin size={12} />
                      </button>
                      <button onClick={() => { if (confirm('Delete folder?')) deleteFolder(folder.id); }}
                  ...
                  <div className="grid grid-cols-3 gap-1 rounded-xl overflow-hidden h-24">
                    {folderImages.slice(0, 3).map(img => (
                      <img key={img.id} src={img.previewUrl || img.imageUrl} className="w-full h-full object-cover" alt="" />
                    ))}
      ...
      {unsortedImages.length > 0 && (
        <section>
          <h4 className="text-sm font-black text-foreground uppercase tracking-wider mb-4">Unsorted ({unsortedImages.length})</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {unsortedImages.map((img, idx) => (
              <GalleryImageCard key={img.id} id={img.id} imageUrl={img.imageUrl} previewUrl={img.previewUrl}
                title={img.tags[0]} tags={img.tags} liked={img.liked} saved={true}
                onView={() => openViewer(img, unsortedImages, idx)}
                onLike={() => img.liked ? unlikeImage(img.id) : likeImage(img.id)} />
            ))}
          </div>
        </section>
      )}
      ...
      {savedImages.length === 0 && folders.length === 0 && (
        <div className="py-20 border-2 border-dashed border-white/5 rounded-[32px] flex flex-col items-center justify-center text-center gap-4 bg-white/[0.01]">
          ...
          <div>
            <p className="text-foreground/40 text-sm font-bold">No saved images yet</p>
            <p className="text-foreground-dim text-xs mt-1">Browse Trending or Discover to start saving</p>
          </div>
        </div>
      )}
    </div>
