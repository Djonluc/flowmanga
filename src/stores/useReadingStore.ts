...
interface ReadingState {
  ...
  openFolder: (
    ...
    seriesId?: string,
    chapterId?: string,
    sequence?: { id: string; path: string; title: string }[],
...
export const useReadingStore = create<ReadingState>((set, get) => ({
  ...
  openFolder: async (path, seriesId, chapterId, sequence, startPageIndex) => {
    ...
    try {
      const metaContent = await invoke<string>("read_file_string", {
    ...
    if (isFlat) {
      ...
      if (startPageIndex === undefined && chapterId && metadataV3.chapters) {
        const ch = metadataV3.chapters.find(
          (c: any) =>
            c.number === chapterId || `${seriesId}-${c.number}` === chapterId,
        );
  ...
  loadChapter: async (index: number, startPageIndex?: number) => {
    ...
    try {
      ...
      const pages: string[] = await invoke("read_folder", {
        path: chapter.path,
      });
  ...
  setPageIndex: (index) => {
    ...
    if (state.isFlatMode && state.metadata?.chapters) {
      const chIndex = state.metadata.chapters.findIndex(
        (c: any) => safeIndex >= c.startIndex && safeIndex <= c.endIndex,
      );
    ...
    if (state.seriesId) {
      ...
      if (chNumber) {
        import("./useLibraryStore").then((m) => {
          m.useLibraryStore
            .getState()
            .updateReadingProgress(state.seriesId!, chNumber, safeIndex);
        });
      }
    }
  },
